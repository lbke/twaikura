...
const createDisplayName = (user) => {
  ...
  const twitterName = Utils.getNestedProperty(
    user,
    "services.twitter.screenName"
  );
  const linkedinFirstName = Utils.getNestedProperty(
    user,
  ...
  if (linkedinFirstName)
    return `${linkedinFirstName} ${Utils.getNestedProperty(
      user,
...
const ownsOrIsAdmin = (user, document) => {
  return (
    getCollection("Users").owns(user, document) ||
...
const schema = {
  ...
  emails: {
    ...
    onCreate: ({ document }) => {
      ...
      if (!document.emails && document.email)
        return [{ address: document.email }];
    },
  },
  "emails.$": {
    type: Object,
...
/*
  email: {
    ...
    onCreate: ({ document: user }) => {
      ...
      const meteorEmails = Utils.getNestedProperty(
        user,
        "services.meteor-developer.emails"
      );
      const facebookEmail = Utils.getNestedProperty(
        user,
        "services.facebook.email"
      );
      const githubEmail = Utils.getNestedProperty(
        user,
        "services.github.email"
      );
      const googleEmail = Utils.getNestedProperty(
        user,
        "services.google.email"
      );
      const linkedinEmail = Utils.getNestedProperty(
        user,
        "services.linkedin.emailAddress"
      );

      if (meteorEmails)
        return _.findWhere(meteorEmails, { primary: true }).address;
...
/*
  slug: {
    ...
    onCreate: ({ document: user }) => {
      ...
      return basicSlug
        ? Utils.getUnusedSlugByCollectionName("Users", basicSlug)
        : user._id;
    },
  },
  /**
